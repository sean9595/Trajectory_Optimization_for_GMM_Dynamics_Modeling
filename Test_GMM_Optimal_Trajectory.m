...
grid on
%%
plot(theta_opt,output_opt);
%%
% n x m 행렬, n은 데이터의 차원, m은 데이터의 길이
Data_temp = [theta';output']; 
Data = gpuArray(Data_temp);
%%
% n x m 행렬, n은 데이터의 차원, m은 데이터의 길이
% Data_temp = [angle_out_J01';acc_out_J01';trq_in_J01']; 
Data_temp = [theta_opt';output_opt']; 
Data = gpuArray(Data_temp);
%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
simout02=sim('sinusoidal_mass_CL_val.slx');
...
figure(4)
hold on;
plot3(DataIn(1,:),DataIn(2,:),expData,'.','markersize',3,'color',[1.0 .0 .0]);
plot3(DataIn(1,:),DataIn(2,:),trq_in_J01_val,'.','markersize',3,'color',[0.0 1.0 .0]);
grid on
%%
figure(10)
hold on
...
grid on
%%
err_torq01 = trq_in_J01_val' - expData(1,:);
