function cost = Test_Kmeans_cost_function_Reduced_Param(delta, N, means, Ts, NofMultipleSign)
    %Param designate
    for i=1:NofMultipleSign
        a_cost(i) = delta(1+3*(i-1));
        b_cost(i) = delta(2+3*(i-1));
        omega_cost(i) = delta(3+3*(i-1));
    end
    bias_cost = delta(3*NofMultipleSign+1);
